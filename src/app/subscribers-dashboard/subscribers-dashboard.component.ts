...
export class SubscribersDashboardComponent implements OnInit, AfterViewInit {
    ...
    this.afAuth.authState.subscribe((user) => {
      if (user) {
        ...
        this.subscribersService
          .getSubscribersListForUser(this.userId)
          .subscribe((res) => {
  ...
  toggleEditSave() {
    if (this.editMode && this.editingRow) {
      const subscriberToUpdate = this.dataSource.data.find(
        (s) => s.id === this.editingRow
      );
      if (subscriberToUpdate) {
        this.subscribersService
          .updateSubscriber(subscriberToUpdate.id, {
  ...
  deleteSelected() {
    ...
    if (selectedIds.length > 1) {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (result) {
          this.subscribersService
            .deleteMultipleSubscribers(selectedIds)
            .then(() => {
      ...
      this.subscribersService
        .deleteSubscriber(selectedIds[0])
        .then(() => {
  ...
  confirmSendText() {
    const unsubscribeMessage = "\n\nText UNSUBSCRIBE to Unsubscribe";
    const fullMessage = this.messageText + unsubscribeMessage;
    const totalMessagesCount =
      Math.ceil(fullMessage.length / 153) * this.selection.selected.length;
    ...
    Promise.all(promises)
      .then(() => {
        this.authService
          ...
          .catch((error) => {
            this._snackBar.open(
              "Failed to update textsThisMonth: " + error.message,
              "Close",
              {
                horizontalPosition: this.horizontalPosition,
                verticalPosition: this.verticalPosition,
      ...
      .catch((error) => {
        this._snackBar.open(
          "Failed to send some text messages: " + error.message,
          "Close",
          {
            horizontalPosition: this.horizontalPosition,
            verticalPosition: this.verticalPosition,
