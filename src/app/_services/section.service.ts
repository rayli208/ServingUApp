...
export class SectionService {
  ...
  getSectionDoc(id: string) {
    return this.afs.collection("sections").doc(id).valueChanges();
  }

  // Get list of sections for a specific user, ordered by 'order'
  getSectionsListForUser(userId: string) {
    return this.afs.collection<Section>("sections", ref => ref.where('uid', '==', userId).orderBy('order')).snapshotChanges();
  }

  // Create a new section
  createSection(section: Section) {
    return this.afs.collection("sections").add(section);
  }
